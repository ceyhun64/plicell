...
export default function Vertical() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        const res = await fetch("/api/products/category/9");
    ...
    <div className="max-w-8xl mx-auto mb-20">
      ...
      <div className="relative w-full h-[50vh] md:h-[70vh] md:mb-12 overflow-hidden">
        ...
        <div className="absolute inset-0">
          <Image
            src="/categories/drapes.webp"
            alt="Fon Perde Banner"
            fill
            className="object-cover"
            sizes="100vw"
          />
          {/* Modern Gradient Overlay */}
          <div className="absolute inset-0 bg-gradient-to-br from-black/80 via-rose-950/50 to-transparent"></div>
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div>
        </div>

        {/* Decorative Elements */}
        <div className="absolute top-0 right-0 w-48 h-48 sm:w-72 sm:h-72 bg-rose-500/10 rounded-full blur-3xl"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 sm:w-72 sm:h-72 bg-rose-700/10 rounded-full blur-3xl"></div>
        ...
        <div className="relative h-full flex flex-col justify-end px-4 sm:px-6 md:px-12 lg:px-16 pb-8 sm:pb-12 md:pb-16 max-w-3xl md:max-w-7xl mx-auto">
          ...
          <div className="mb-4 sm:mb-6 inline-flex items-center gap-2 w-fit">
            ...
            <span className="text-rose-400 text-xs sm:text-sm md:text-base font-medium tracking-widest uppercase">
              Lüks Kumaşlar
            </span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-7xl lg:text-8xl font-bold text-white mb-3 sm:mb-4 md:mb-6 tracking-tight">
            Fon Perde
            <span className="block text-lg sm:text-xl md:text-4xl lg:text-5xl font-light text-rose-200 mt-1 sm:mt-2 md:mt-4">
              Klasik Zarafet
            </span>
          </h1>
          ...
          <p className="text-white/90 text-sm sm:text-base md:text-lg lg:text-xl max-w-full sm:max-w-2xl mb-6 sm:mb-8 leading-relaxed">
            Kaliteli kumaşlar ve zengin renk seçenekleriyle mekanlarınıza değer
            katan fon perde koleksiyonumuzu keşfedin.
          </p>
